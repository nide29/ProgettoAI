...
class LoadBoxInWs : public plansys2::ActionExecutorClient
{
public:
  LoadBoxInWs()
  : plansys2::ActionExecutorClient("loadboxinws", 250ms)
  {
    progress_ = 0.0;
  }
  ...
  { std :: vector <std :: string > arguments = get_arguments () ;
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Robot "+arguments [0]+ " is filling "+ 
            arguments [1]+ " in "+ arguments [2] + " with some "+
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<LoadBoxInWs>();

  node->set_parameter(rclcpp::Parameter("action_name", "LOADBOXINWS"));
